...
export default function Process() {
  ...
  const getStepIcon = (stepName) => {
    switch (stepName.toUpperCase()) {
      case 'GENESIS': return <Zap className="text-cyan-400" size={20} />;
      ...
      case 'INTERNSHIP': return <Briefcase className="text-cyan-400" size={20} />;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none z-0">
        <div className="absolute top-[20%] right-[-5%] w-[400px] h-[400px] bg-cyan-400/5 dark:bg-cyan-400/5 blur-3xl rounded-full" />
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-16 lg:px-20 relative z-10 w-full">
        ...
        <div className="flex flex-col items-center gap-6 mb-16 text-center">
          <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-terracotta/10 border border-terracotta/20 dark:bg-cyan-500/15 dark:border-cyan-500/30 text-terracotta dark:text-cyan-400 text-[9px] font-black uppercase tracking-[0.25em] w-fit">
            <Milestone size={10} />
            Strategic Roadmap
          </div>
          ...
          <h2 className="font-heading text-4xl md:text-5xl font-bold uppercase tracking-tight leading-[1.05] text-zinc-900 dark:text-white max-w-2xl">
            My <span className="text-terracotta dark:text-cyan-400 italic font-heading underline decoration-[3px] underline-offset-8">Process</span>.
          </h2>
          ...
          <div className="flex flex-wrap items-center justify-center gap-8 border border-zinc-900/5 dark:border-white/5 bg-zinc-900/[0.02] dark:bg-white/[0.02] px-8 py-4 rounded-3xl backdrop-blur-sm mt-4">
            <div className="text-center px-4">
              <span className="block text-2xl font-black font-heading text-terracotta dark:text-cyan-400">10+</span>
            ...
            <div className="text-center px-4">
              <span className="block text-2xl font-black font-heading text-terracotta dark:text-cyan-400">B.Tech</span>
              <span className="text-[9px] font-black uppercase tracking-wider text-zinc-400 dark:text-zinc-500">SASTRA CSE</span>
            </div>
            <div className="h-8 w-[1px] bg-zinc-900/10 dark:bg-white/10" />
            <div className="text-center px-4">
              <span className="block text-2xl font-black font-heading text-terracotta dark:text-cyan-400">AIR 5005</span>
              ...
              {currentRoadmap.map((stepItem, idx) => (
                <button
                  ...
                  className={`flex items-center gap-3.5 px-5 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest text-left transition-all duration-300 cursor-pointer ${
                    activeStep === idx
                      ? 'bg-zinc-900 text-beige dark:bg-[#f5f1e8] dark:text-charcoal shadow-md scale-102 border-l-4 border-l-terracotta dark:border-l-cyan-400'
                ...
                >
                  <span className="font-heading font-black italic opacity-60 text-xs text-terracotta dark:text-cyan-400">
                    {stepItem.step}
                  </span>
              ...
              {currentRoadmap.map((stepItem, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className="glass-card p-6 md:p-10 border border-zinc-900/5 dark:border-white/5 hover:border-terracotta/20 dark:hover:border-cyan-400/20 shadow-md relative overflow-hidden"
                  >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 mb-6 border-b border-zinc-900/5 dark:border-white/5">
                      <div className="flex items-center gap-3.5">
                        <div className="px-4 py-2 rounded-2xl bg-terracotta/10 border border-terracotta/20 dark:bg-cyan-500/15 dark:border-cyan-500/30 text-terracotta dark:text-cyan-400 text-xs font-black uppercase tracking-widest flex items-center gap-2">
                      ...
                      <div className="text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500 flex items-center gap-1.5 bg-zinc-900/5 dark:bg-white/5 px-4 py-2 rounded-2xl">
                        <span>Speed:</span>
                        <span className="text-terracotta dark:text-cyan-400">{stepItem.timeline}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid md:grid-cols-12 gap-8 items-start">
                          ...
                          <ul className="space-y-3 text-xs text-zinc-600 dark:text-zinc-400 font-medium leading-relaxed">
                            {stepItem.highlights.map((highlight, hIdx) => (
                              <li key={hIdx} className="flex gap-2.5 items-start">
                                <CheckCircle2 size={14} className="text-terracotta dark:text-cyan-400 shrink-0 mt-0.5" />
                                <span>{highlight}</span>
                              </li>
                            ))}
                          </ul>
                        ...
                        <div className="space-y-2.5 pt-2">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {stepItem.techTags.map((tag, tIdx) => (
                              <span 
                                key={tIdx}
                                className="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg bg-zinc-900/5 dark:bg-white/5 border border-zinc-900/5 dark:border-white/5 text-zinc-500 dark:text-zinc-400 hover:text-terracotta dark:hover:text-cyan-400 transition-colors"
                              >
                                {tag}
                              </span>
                            ))}
                      ...
                      <div className="md:col-span-5 flex justify-center items-center h-full min-h-[220px]">
                        <div className="relative w-48 h-48 rounded-3xl bg-zinc-900 dark:bg-black border border-zinc-900/10 dark:border-white/10 shadow-2xl flex flex-col justify-center items-center overflow-hidden">
                          ...
                          {/* Node Dots */}
                          <div className="absolute top-10 left-10 w-2.5 h-2.5 rounded-full bg-terracotta/40 dark:bg-cyan-400/40 animate-ping" />
                          <div className="absolute top-10 left-10 w-1.5 h-1.5 rounded-full bg-terracotta dark:bg-cyan-400" />
                          ...
                          <motion.div 
                            ...
                            className="w-16 h-16 rounded-2xl bg-zinc-950 dark:bg-zinc-900/90 border border-terracotta/30 dark:border-cyan-400/30 shadow-[0_0_20px_rgba(56,189,248,0.2)] flex items-center justify-center relative z-10"
                          >
                            {getStepIcon(stepItem.phase)}
                          </motion.div>

                          {/* Captions */}
                          <span className="text-[8px] font-black uppercase tracking-[0.25em] text-terracotta dark:text-cyan-400 mt-4 relative z-10">
                            {stepItem.phase}
                          </span>
